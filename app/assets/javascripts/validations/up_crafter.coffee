class @UpCrafter
  ...
  validate_registered_crafter_form: () ->
    if @utils.check_field('crafter[company_name]') && @utils.check_field('crafter[email]') && @utils.check_field('crafter[password]') && @utils.check_field('crafter[password_confirmation]')
      if $('#category_id').val() != ''
